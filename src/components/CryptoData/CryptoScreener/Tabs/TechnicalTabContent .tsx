...
const TechnicalTabContent: React.FC<TechnicalTabContentProps> = ({ onSelectionChange, selectedValues }) => {
  const { filters, loading, error } = useFilters(`${process.env.PUBLIC_URL}/Filters.json`);
  const { control } = useForm<FieldValues>();
  const [technicalOptions, setTechnicalOptions] = useState<{ [key: string]: { options: string[], label: string, description: string } }>({});

  useEffect(() => {
    if (filters.Technical) {
      setTechnicalOptions(filters.Technical);
    }
  }, [filters]);
  ...
  return (
    <Box pl={2} pt={2}>
      <Grid container spacing={2} sx={{ backgroundColor: '#f7f8f8', maxHeight: '370px', overflowY: 'scroll' }}>
        {Object.keys(technicalOptions).map((indicator) => (
          <Grid item xs={12} sm={6} md={12 / columnsPerRow} key={indicator}>
            <Box mb={2} width="100%">
              <Typography style={{ fontSize: '0.75rem', marginBottom: '4px', display: 'flex', alignItems: 'center' }}>
                <span>{technicalOptions[indicator].label}:</span>
                {technicalOptions[indicator].description && (
                    <Tooltip title={technicalOptions[indicator].description} arrow>
              ...
              <Controller
                name={indicator.toLowerCase().replace(/ /g, '_')}
                control={control}
                defaultValue={selectedValues[indicator.toLowerCase().replace(/ /g, '_')] || []}
                render={({ field }) => (
                  <Autocomplete
                    multiple
                    options={technicalOptions[indicator].options}
                    ...
                    value={field.value || []}
                    onChange={(_, value) => {
                      handleSelectionChange(indicator.toLowerCase().replace(/ /g, '_'), value as string[]);
