...
interface QuantitativeTabContentProps {
  onSelectionChange: (fieldName: string, values: string[]) => void;
  selectedValues: { [key: string]: string[] };
}

const QuantitativeTabContent: React.FC<QuantitativeTabContentProps> = ({ onSelectionChange, selectedValues }) => {
  const { filters, loading, error } = useFilters(`${process.env.PUBLIC_URL}/Filters.json`);
  const { control } = useForm<FieldValues>();
  const [quantitativeOptions, setQuantitativeOptions] = useState<{ [key: string]: { options: string[], label: string, description: string } }>({});

  useEffect(() => {
    if (filters.Quantitative) {
      setQuantitativeOptions(filters.Quantitative);
    }
  }, [filters]);
  ...
  return (
    <Box pl={2} pt={2}>
      <Grid container spacing={2} sx={{ backgroundColor: '#f7f8f8', maxHeight: '370px', overflowY: 'scroll' }}>
        {Object.keys(quantitativeOptions).map((column) => (
          <Grid item xs={12} sm={6} md={12 / columnsPerRow} key={column}>
            <Box mb={2} width="100%">
              <Typography style={{ fontSize: '0.75rem', marginBottom: '4px', display: 'flex', alignItems: 'center' }}>
                <span>{quantitativeOptions[column].label}:</span>
                {quantitativeOptions[column].description && (
                    <Tooltip title={quantitativeOptions[column].description} arrow>
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Autocomplete
                    multiple
                    options={quantitativeOptions[column].options}
...
export default QuantitativeTabContent;
