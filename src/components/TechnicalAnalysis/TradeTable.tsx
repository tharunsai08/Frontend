...
const TradeTable: React.FC<TradeTableProps> = ({ data }) => {
  return (
    <Card variant="outlined" style={{ marginBottom: '20px', backgroundColor: '#f0f0f0',boxShadow: '0 4px 8px 0 #c6f5e4, 0 6px 20px 0 #c6f5e4'  }}>
      <Typography variant="h6" style={{ color: '#002060', fontWeight: 'bold', padding: '10px', marginBottom: '10px' }}>
        Trade Table :    <span style={{ color: getActionColor(data.trade_action_count), fontWeight: 'bold' }}>
        {data.trade_action_count}    </span>
      </Typography>
      <Card variant="outlined" style={{ margin: '10px', marginTop: '0', backgroundColor: '#FDFEF7' }}>
        <TableContainer>
          <Table aria-label="Trade Table">
            ...
            <TableBody>
              {data.trade_table.map((row: any, index: number) => (
                <TableRow key={index}>
